...
import {
  ...
  getIndent,
  isAlpha,
  isAlphaOrDigitOrLowDashOrDashOrQuestion,
  isDigit,
  isKeyword,
  isWhiteSpaceOrTab,
  KeywordsToTokenKind
    ...
    function worker() {
      ...
      switch (ch) {
        ...
        case Chars.Semi: {
          let i = 0;
          while (
            current + i < text.length &&
            text[current + i] !== Chars.LineFeed
          ) {
            i++;
          }
          current += i;
        ...
        case Chars.Comma:
          current++;
          token = finishNode(
            createToken(CharsToTokenKind[ch]),
            tokenStart,
            current
          );
          break;
        case Chars.LessThan:
          if (current + 1 < text.length && text[current + 1] === Chars.Equals) {
            current += 2;
            token = finishNode(
              createToken(SyntaxKind.LessEqualsThanToken),
              tokenStart,
              current
            );
            break;
          ...
          current++;
          token = finishNode(
            createToken(SyntaxKind.LessThanToken),
            tokenStart,
            current
          );
          break;
        case Chars.GreaterThan:
          if (current + 1 < text.length && text[current + 1] === Chars.Equals) {
            current += 2;
            token = finishNode(
              createToken(SyntaxKind.GreaterEqualsThanToken),
              tokenStart,
              current
            );
            break;
          ...
          current++;
          token = finishNode(
            createToken(SyntaxKind.GreaterThanToken),
            tokenStart,
            current
          );
          break;
        ...
        case Chars.Quote: {
          ...
          while (
            current + i < text.length &&
            text[current + i] !== Chars.Quote
          ) {
            if (
              text[current + i] === Chars.BackSlash &&
              current + i + 1 < text.length &&
              text[current + i + 1] === Chars.Quote
            ) {
              i++;
            }
            i++;
          ...
          token = finishNode(
            createStringLiteralToken(value),
            tokenStart,
            current
          );
          break;
        ...
        case Chars._9: {
          ...
          token = finishNode(
            createNumberLiteralToken(value),
            tokenStart,
            current
          );
          break;
        ...
        default:
          if (isAlpha(ch)) {
            ...
            while (
              current + i < text.length &&
              isAlphaOrDigitOrLowDashOrDashOrQuestion(text[current + i])
            ) {
              i++;
            }
            current += i;
            ...
            if (isKeyword(value)) {
              token = finishNode(
                createToken(KeywordsToTokenKind[value]),
                tokenStart,
                current
              );
              break;
